...
import validateReceipts, { throwAppropriateError } from "../../middleware/validations";
...
describe('validations', () => {
    it('should not call next with an error if validation is successful', () => {
        const result = validateReceipts(request as Request, response as Response, next);
        expect(next).toHaveBeenCalledWith();
    });
    it ('should call next with an appropriate error if total price validation fails', () => {
        let result = validateReceipts({...request, body:{...request.body, total: 'a'}} as Request, response as Response, next);
        expect(next).toHaveBeenCalledWith(new ApiError({}, 400, 'invalid total price'));
        expect(result).not.toBeDefined();
    });
    it ('should call next with an appropriate error if retailer validation fails', () => {
        let result = validateReceipts({...request, body:{...request.body, retailer: '....'}} as Request, response as Response, next);
        expect(next).toHaveBeenCalledWith(new ApiError({}, 400, 'invalid retailer'));
        expect(result).not.toBeDefined();
    });
    it ('should call next with an appropriate error if purchase date validation fails', () => {
        let result = validateReceipts({...request, body:{...request.body, purchaseDate: '....'}} as Request, response as Response, next);
    ...
    it ('should call next with an appropriate error if purchase time validation fails', () => {
        let result = validateReceipts({...request, body:{...request.body, purchaseTime: '....'}} as Request, response as Response, next);
        expect(next).toHaveBeenCalledWith(new ApiError({}, 400, 'invalid purchase time'));
        expect(result).not.toBeDefined();
    });
    it ('should call next with an appropriate error if item price validation fails', () => {
        let result = validateReceipts({...request, body:{...request.body, items:[{ shortDescription: "description",
        price: "*"}]}} as Request, response as Response, next);
        expect(next).toHaveBeenCalledWith(new ApiError({}, 400, 'invalid item price'));
        expect(result).not.toBeDefined();
    });
    it ('should call next with an appropriate error if item short descriptiom validation fails', () => {
        let result = validateReceipts({...request, body:{...request.body, items:[{ shortDescription: "0 * ",
        price: "11.20"}]}} as Request, response as Response, next);
        expect(next).toHaveBeenCalledWith(new ApiError({}, 400, 'invalid short description for some of the items'));
        expect(result).not.toBeDefined();
    });
});
describe('throwAppropriateError', () => {
    it ('should return appropriate api error for each case', () => {
        ...
        try {
            ...
            expect(error).toEqual(new ApiError({}, 400, "invalid short description for some of the items"));
        }
